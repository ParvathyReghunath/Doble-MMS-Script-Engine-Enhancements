...
/* --------  ---  ------   -------------------------------------------  */
/* 06/04/18  EJV  Added slog_add_filter.                                */
/* 06/04/18  EJV           slog_get_filter: ret enable=SD_TRUE if       */
/*                         filtered logging not enables.                */
/* 05/01/18  MDE           Added slog_get_filter                        */
/* 02/27/13  JRB           Del MSDOS code.                              */
/* 09/07/12  EJV           AIX slogSetHdr: log threadId one time.       */
/* 07/26/12  EJV           Linux,Sun slogSetHdr: log threadId one time. */
...
#if defined(_WIN32) || defined(sun) || defined(linux) || defined(_AIX)
...
#elif defined(sun) || defined(linux) || defined(_AIX)
      threadId = GET_THREAD_ID();
      sprintf (tmp, "(%s %d ThreadId=" S_FMT_THREAD_IDX " ) ",
               fname, lineNum, threadId);
#else
...
ST_BOOLEAN slog_get_filter (LOG_CTRL *lc, ST_CHAR *filter, ST_BOOLEAN *presentOut)
  {
  ST_BOOLEAN enabled = SD_FALSE;
  ST_INT i;

  if (presentOut != NULL)
    *presentOut = SD_FALSE;

  for (i = 0; i < lc->filter_name_count; ++i)
    {
    if (!strcmp (filter, lc->filter_names[i]))
      {
      /* The filter is present */
      if (presentOut != NULL)
        *presentOut = SD_TRUE;

      enabled = lc->filtered_logging_enable;
      break;
      }
    }

  if (!lc->filtered_logging_enable)
    enabled = SD_TRUE;     /* log as usual */       
...
/************************************************************************/
/*                      slog_add_filter                                 */
/* This fun is used to auto-add IPs for remote AR Names.                */
/************************************************************************/
...
#if 0 /*By Arun*/
  if (lc->filter_name_count < sizeof (lc->filter_names) / sizeof (ST_CHAR *))
    {
    lc->filter_names[lc->filter_name_count] = chk_strdup (filter);
    ++lc->filter_name_count;
    }
  else
    rtn = SD_FAILURE;
#endif
