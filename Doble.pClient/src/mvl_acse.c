...
/* --------  ---  ------   -------------------------------------------  */
/* 01/14/19  JRB  mvla_initiate_req_exx: set aarq_apdu.log_enable.      */
/* 07/10/18  EJV  mvla_initiate_req_exx: set encryptionUsed,authTypeUsed*/
/* 04/13/17  JRB  u_a_abort_ind: close files in "file_array" too.       */
/* 02/06/17  JRB  u_a_associate_ind: chg result_diag to 1 (no-reason-given).*/
/* 09/27/16  JRB  mvla_initiate_req_exx: add semaphore.                 */
/*                mvl_abort_req_ex: add semaphore & check "in_use".     */
/*                u_a_release_cnf: chg order to avoid crash w/ AA_SPEC DatSet.*/
/*                mvl_add_bind_ctrl: fix warning "activate" never used. */
/* 07/27/16  EJV  mvl_abort_req_ex: on Abort Req call mvl_clr_ind_que.  */
/* 03/02/16  JRB  On Abort or Release Ind, call mvl_clr_ind_que.        */
/* 12/08/15  EJV  u_a_associate_ind: call _mms_chk_initiate_params      */
/*                  before calling user ind function.                   */
/*                u_a_associate_cnf: dec Cnf PDU before calling user fun*/
/* 01/29/15  JRB  Chk params on Initiate Indication and Confirm.        */
/*                u_a_associate_cnf: never set info->result (no impact).*/
/* 07/23/14  EJV  u_a_associate_cnf: chk arAuthMode==S_AR_AUTH_MODE_MACE*/
/* 04/16/14  EJV  u_a_associate_cnf:if con failed,reset authHashDetected*/
/* 10/09/13  EJV  mvl_start_acse: added log when initialization failed. */
/* 09/11/13  EJV  ma_bind (ICCP_LITE): check for COPP_ERR_ALREADY_BOUND.*/
/*                  Added local AR Name in logging.                     */
/*                ma_get_calling_conn_ctrl: init authHash, partAuthHash.*/
...
#include "copp_usr.h"           /* for COPP_ERR_ALREADY_BOUND   */
#include "mi.h"
#endif
...
ST_RET mvl_start_acse (MVL_CFG_INFO *cfgInfo)
  ...
  if (!cfgInfo->use_bind_ctrl)
    ...
    cfgInfo->bind_ctrl = chk_calloc (cfgInfo->max_bind_ctrl, sizeof (NET_BIND_CTRL));
  ...
  *mvl_cfg_info = *cfgInfo;     /* copy caller's struct (includes allocated bind_ctrl)*/
  ...
  if (ret == SD_SUCCESS)
    {
    mvl_acse_ok = SD_TRUE;
    MVL_LOG_ACSE0 ("ACSE: Start completed successfully.");
    }
  else
    {
    copp_terminate ();
    MVL_LOG_ACSE1 ("ACSE: Start failed, returning '0x%x'", ret);
    }
...
ST_INT mvla_initiate_req_exx (ST_CHAR *locAr,
  ...
  /* Protect access to mvl_calling_conn_ctrl, mmsl_enc_buf, and stack.*/
  S_LOCK_COMMON_RESOURCES ();
  ...
  *req_out = req;
  ...
  if (rc == SD_SUCCESS)
    ...
    if (locDe == NULL)
      ...
      /* Set log_enable. This affects logging only if libraries compiled with FILTERED_LOGGING defined.*/
      aarq_apdu.log_enable = slog_get_filter (sLogCtrl, remAr, NULL);
#endif
  ...
  if (rc == SD_SUCCESS)
    ...
    *net_info_out = cc;
    ...
    /* set these fields for easy access later */
    if (encrypt_info)
      cc->encryptionUsed = encrypt_info->encryptMode;
    else
      cc->encryptionUsed = SD_FALSE;
    cc->authTypeUsed = S_AR_AUTH_MODE_UNAUTHENTICATED;
    if (auth_info && auth_info->auth_pres)
      {
      if (auth_info->mech_type == ACSE_AUTH_MECH_MACE_CERT)
        cc->authTypeUsed = S_AR_AUTH_MODE_MACE;
      else if (auth_info->mech_type == ACSE_AUTH_MECH_PASSWORD)
        cc->authTypeUsed = S_AR_AUTH_MODE_PASSWORD;
      }
    }
  else
    {
...
ST_RET mvl_abort_req_ex (MVL_NET_INFO *cc,
  ...
  S_LOCK_COMMON_RESOURCES (); /* protect access to stack  */

  MVL_LOG_ACSE1 ("ACSE: sending abort for MVL_NET_INFO " S_FMT_PTR, cc);

  if (!cc->in_use)
    {
    /* Connection closed so cc->acse_conn_id is invalid & a_abort_req would fail.*/
    /* NOTE: This may happen if remote device aborted first.  */
    MVL_LOG_ERR0 ("ACSE: Connection closed. Invalid state to send abort.");
    S_UNLOCK_COMMON_RESOURCES ();
    return (MVL_ERR_CNF_DISCONNECTED);
    }
  ...
  /* CRITICAL: free any pending indications.    */
  mvl_clr_ind_que (cc);
  if (cc->numpend_ind != 0)     /* sanity check */
    MVL_LOG_ERR1 ("ERROR: numpend_ind = %d after ACSE Abort.Req. Should be 0.", cc->numpend_ind);
...
static ST_RET ma_bind (NET_BIND_CTRL *bc, ST_INT timeout)
  ...
  if (ret != SD_SUCCESS)
    {
#if defined(ICCP_LITE)
    /* NOTE:       Added this check because ICCP-Lite may configure two local AR Names    */
    /* Sep-11-2013 with the same PSEL,SSEL,TSEL but different other pres_addr components. */
    /*             The mi_match_assoc will match the exact Local AR Name in connection.   */
    if (ret == COPP_ERR_ALREADY_BOUND)
      {
      MVL_LOG_ACSE1 ("ACSE: bind OK for Local AR name='%s' (selector set already bound)", bc->ar_name);
      ret = SD_SUCCESS;  /* assume successful bind */
      }
    else
#endif /* defined(ICCP_LITE) */
      {
    ...
    MVL_LOG_ACSE1 ("ACSE: bind OK for Local AR name='%s'", bc->ar_name);
...
ST_VOID u_a_associate_ind (ST_VOID *u_bind_id, ST_VOID *acse_conn_id,
  ...
  ST_INT16 init_err_code;  /* error code for mpl_init_err (Initiate-ErrorPDU)*/

  init_err_code = -1;  /* init to illegal value. Use only if code below changes it.*/
  ...
  if (cc)
    ...
    if (rslt.err_code == NO_DECODE_ERR && rslt.type == MMSINITREQ)
      ...
      ret = _mms_chk_initiate_params (init_req, &init_err_code);
      if (ret != SD_SUCCESS)
        {
        MVL_LOG_ERR0 ("MMS Initiate Indication Error: invalid parameters");
        ++mvl_mms_statistics.calledConnFail;
        /* leads to error response below. */
        }
      ...
      if (ret == SD_SUCCESS)
        ...
        /* NOTE: If (init_req->max_segsize_pres==SD_FALSE), proposed size is "unlimited".*/
        /* We will send our default value (set above) in response.*/
      ...
      if (ret != SD_SUCCESS)
        ...
        if (init_err_code != -1) /* code above changed it  */
          ret = mpl_init_err (8, init_err_code);
        else
...
ST_VOID u_a_associate_cnf (ST_VOID *acse_conn_id, ST_VOID *user_conn_id,
  ...
  ST_RET result;
  ST_ACSE_AUTH auth_ret;
  ST_INT16 init_err_code; /* set by _mms_chk_initiate_params but not used */
  ABRT_APDU abrtinfo;

  /* Init Abort info just in case it is needed. */
  abrtinfo.source = 0;          /* user */
  abrtinfo.diagnostic_pres = SD_TRUE;
  abrtinfo.diagnostic = ACSE_DIAG_NO_REASON;
  abrtinfo.user_info.len = 0;
  ...
  if (!req)
    ...
    /* If ACSE connection succeeded, must Abort it. */
    if (info->result == SD_SUCCESS)
      a_abort_req (acse_conn_id, &abrtinfo);
    cc->in_use = SD_FALSE;
    return;
    }
  ...
  if (info->result != SD_SUCCESS)
    ...
    S_SEC_LOC_REM_AR   *remArSec = cc->rem_ar_sec;

    /* NOTE:                                                                     */
    /* the cc->rem_ar_sec maybe NULL in case AXS4-61850 accepted connection w/o  */
    /* authenticating remote node                                                */
    /* NOTE2:                                                                    */
    /* if secured connection failed (possible hash config problem we don't know  */
    /* from the info->result) to implement the S_AR_AUTH_SIGNATURE_AUTO algorithm*/
    /* check if the failure was on the first connect attempt. We are assuming    */
    /* that we used the remArSec->authHashToUse=S_AR_AUTH_HASH_NONE on a first   */
    /* attempt.                                                                  */
    if (secManAccessCfg (&pSecCfg) == SD_SUCCESS)
      {
      if (pSecCfg->secureModeEnabled && remArSec && remArSec->arAuthMode == S_AR_AUTH_MODE_MACE)
        {
        if (remArSec->authSignatureMethod == S_AR_AUTH_SIGNATURE_AUTO && !remArSec->authHashDetected)
          {
          if (remArSec->authHashToUse == S_AR_AUTH_HASH_NONE)  /* first connect ?  */
            {
            remArSec->authHashToUse = remArSec->authHash;  /* next time try configured hash */
#if defined(ICCP_LITE)
              {
              MI_ASSOC_CTRL *mi_assoc = (MI_ASSOC_CTRL *) cc->mi_conn->mi_assoc;
              /* use authHashErr to prevent rolling to the next remote AR Name    */
              mi_assoc->authHashErr = S_SEC_CHECK_ERR_MACE_CERT_SIG_TIME_NOT_VERIFIED;
              }
#else
            ;  /* DEBUG MMS_LITE: any special handling needed here ? */
#endif
            }
          else
            /* probably second connect attempt */
            remArSec->authHashToUse = S_AR_AUTH_HASH_NONE; /* back to None  */
          }
        else if (remArSec->authSignatureMethod == S_AR_AUTH_SIGNATURE_AUTO && remArSec->authHashDetected)
          {
          /* the conenction failed, maybe the hashing algorithm changed, reset to detect the algorithm again */
          remArSec->authHashDetected = SD_FALSE;
          remArSec->authHashToUse = S_AR_AUTH_HASH_NONE; /* back to None  */
#if defined(ICCP_LITE)
            {
            MI_ASSOC_CTRL *mi_assoc = (MI_ASSOC_CTRL *) cc->mi_conn->mi_assoc;
  ...
  if (info->result == SD_SUCCESS)
    ...
    if (rslt.err_code == NO_DECODE_ERR)
      ...
      switch (rslt.type)
        ...
        case MMSINITRESP :
          ...
          if (_mms_chk_initiate_params (init_resp, &init_err_code) != SD_SUCCESS)
            {
            MVL_LOG_ERR0 ("MMS Initiate Confirm Error: invalid parameters");
            result = SD_FAILURE;  /* causes ACSE Abort below */
            }
          else if (init_resp->max_segsize_pres == SD_FALSE ||
              /* this means PDU size is "unlimited". We never propose this */
              /* in request, so this is not allowed in response. */
              init_resp->maxreq_called > cc->locl_init_info.maxreq_called ||
              init_resp->maxreq_calling > cc->locl_init_info.maxreq_calling ||
              init_resp->max_nest > cc->locl_init_info.max_nest ||
              init_resp->max_segsize > cc->locl_init_info.max_segsize ||
              init_resp->version > cc->locl_init_info.version)
            {
            MVL_LOG_ERR0 ("MMS Initiate Response protocol error: Negotiated value > Proposed value");
            result = SD_FAILURE;  /* causes ACSE Abort below */
            }
          else
            {
    ...
    if (result != SD_SUCCESS)
      {
      /* Something failed above, but ACSE is connected. Must abort ACSE connection.  */
      a_abort_req (acse_conn_id, &abrtinfo);
      cc->in_use = SD_FALSE;
      ++mvl_mms_statistics.callingConnFail;
      }

    /* check ACSE authentication only if this is a positive connect confirm */
    if (result == SD_SUCCESS)
      {
...
ST_VOID u_a_release_ind (ST_VOID *user_conn_id, RLRQ_APDU *info)
  ...
  mvl_clr_ind_que (cc);
  if (cc->numpend_ind != 0)     /* sanity check */
    MVL_LOG_ERR1 ("ERROR: numpend_ind = %d after ACSE Release.Ind. Should be 0.", cc->numpend_ind);
...
ST_VOID u_a_abort_ind (ST_VOID *user_conn_id, ABRT_APDU *info)
  ...
  /* NOTE: new apps should NOT use "file_list", but need this to clean up if it's used.*/
  ...
  /* NOTE: new apps should use "file_array". Need this to clean up "file_array".*/
  /* If user didn't close all files in "file_array" close them now. */
  for (j = 0; j < MAX_FILES_OPEN_PER_CONN; j++)
    {
    if (cc->file_array [j])
      {
      MVL_LOG_ERR0 ("Received Abort indication with file open. Closing now.");
      fclose (cc->file_array [j]);
      cc->file_array [j] = NULL;  /* CRITICAL: clear this array entry  */
      }
    }
  ...
  mvl_clr_ind_que (cc);
  if (cc->numpend_ind != 0)     /* sanity check */
    MVL_LOG_ERR1 ("ERROR: numpend_ind = %d after ACSE Abort.Ind. Should be 0.", cc->numpend_ind);
...
ST_VOID u_a_release_cnf (ST_VOID *user_conn_id, RLRE_APDU *info)
  ...
  /* This must be BEFORE "_mvl_set_net_req_done" so if any RCB contains */
  /* AA_SPEC DatSet, it is removed from RCB BEFORE "_mvl_set_net_req_done" */
  /* frees it (see dsNvl in MVLU_RPT_CTRL structure). */
  if (_mvlu_rpt_disconnect_rcvd_fun != NULL)
    (*_mvlu_rpt_disconnect_rcvd_fun)(cc);
...
static MVL_NET_INFO *ma_get_calling_conn_ctrl (ST_VOID)
  ...
  for (i = 0; i < mvl_cfg_info->num_calling; ++i, ++ni)
    ...
    if (!ni->in_use && ni->req_pend_list == NULL)
      ...
      ni->loc_mace   = NULL;
      ni->rem_mace   = NULL;
      ni->authHash     = S_AR_AUTH_HASH_NONE;
      ni->partAuthHash = S_AR_AUTH_HASH_NONE;
      ni->certExpirationLastCheckTime = 0.0;
#endif
      return (ni);
      }
    }
