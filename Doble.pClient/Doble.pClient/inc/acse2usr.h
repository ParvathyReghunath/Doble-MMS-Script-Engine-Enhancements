...
/* --------  ---  ------   -------------------------------------------  */
/* 05/09/18  EJV  AARQ_APDU: added log_enable for filtered logging.     */
/* 10/27/17  EJV  PRES_ADDR: add ipv6ScopeId.                           */
/* 01/09/17  JRB  PRES_ADDR: add useIpv6 and ipv6Addr.                  */
...
typedef struct tagPRES_ADDR
  {
  ST_BOOLEAN useIpv6;   /* If SD_TRUE, use "ipv6Addr" in "netAddr". */  
  ST_UINT psel_len;
  ...
  ST_UINT    ipv6ScopeId;  /* Linux: IPv6 only, cfg if ipv6Addr is link-local */
  ST_UINT16  port;
  } PRES_ADDR;
...
typedef struct tagAARQ_APDU
  ...
  ST_BOOLEAN    log_enable;    /* enable filtered logging on this conn  */
  } AARQ_APDU;
