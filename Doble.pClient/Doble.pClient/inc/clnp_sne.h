...
/* --------  ---  ------   -------------------------------------------  */
/* 03/06/17  JRB  Del obsolete clnp_snet_write, clnp_etype_write, clnp_snet_ext_write.*/
/* 03/01/17  JRB  Add ifindex to AD_CTX for linux                       */
/* 07/20/16  DSF           Add pcapCaptureInspectorCtx                  */
/* 06/10/16  DSF           Add device_name to AD_CTX                    */
/* 05/19/16  DSF           Add support for multiple adapter contexts    */
/* 05/18/16  DSF           Add features required by GOOSE Tracker       */
/* 04/07/16  DSF           Add clnp_snet_read_raw, clnp_snet_set_filter */
/* 12/12/14  JRB           Add clnp_snet_rx_all_multicast_or_local.     */
/* 11/25/14  MDE           Added PCAP_CAPTURE_FILE option.              */
/* 07/02/13  JRB           SN_UNITDATA: add rx_timeval.                 */
...
typedef struct sn_unitdata
  ...
  struct timeval rx_timeval;    /* timestamp from PCAP when packet rcvd */
  }SN_UNITDATA;
...
ST_RET clnp_snet_read_raw (
  ST_UCHAR **pFramePtr,     /* Pointer to pointer to complete frame including MACs*/
  size_t *pFrameLen,        /* Pointer to length of complete frame             */
  struct timeval *pTimestamp);  /* Pointer to timestamp                        */
...
ST_RET clnp_snet_set_filter (ST_CHAR *filter);
...
ST_RET clnp_snet_rx_all_multicast_or_local (ST_VOID);
...
ST_RET pcapCaptureFileOpen (ST_CHAR *filename);
ST_RET pcapCaptureEnable (ST_BOOLEAN enable);
ST_RET pcapCaptureFileClose (ST_VOID);
ST_RET pcapCaptureEntry (struct pcap_pkthdr *header, const u_char *data);

extern ST_BOOLEAN (*pcapCaptureInspector) (SN_UNITDATA *sn_req,
                         struct pcap_pkthdr *header, const u_char *data);

#endif

/*----------------------------------------------------------------------*/
/* Context-based interface functions to underlying sub-networks         */
/*----------------------------------------------------------------------*/

/*----------------------------------------------------------------------*/
/* Structure below is used to store adapter-specific information        */
...
typedef struct ad_ctx
  ...
  ST_UINT       netmask;
  ST_VOID       *adhandle;
  ST_CHAR       device_name[256];
#if defined(_WIN32)
  HANDLE        hPktEvent;        /* Event Object to be triggered */
#else
  /* wait_any_event function for other systems needs hPktSock.    */
  int           hPktSock;         /* File descriptor to use in "select"   */
#endif
#if defined(linux)
  int           ifindex;          /* Interface index for "sendto" */
#endif
#if defined(PCAP_CAPTURE_FILE)
  /* capture related elements */
  ST_BOOLEAN (*pcapCaptureInspector) (SN_UNITDATA *sn_req,
          struct pcap_pkthdr* header, const u_char* data);
  ST_BOOLEAN (*pcapCaptureInspectorCtx) (struct ad_ctx *ad_ctx, SN_UNITDATA *sn_req,
  struct pcap_pkthdr* header, const u_char* data);

  ST_BOOLEAN    captureEnabled;
  pcap_t        *capturePd;
  pcap_dumper_t *captureDumper;
#endif
  } AD_CTX;

extern AD_CTX *glb_ad_ctx;

ST_RET clnp_snet_init_ctx (CLNP_PARAM *clnp_param, AD_CTX **ad_ctx);
ST_RET clnp_snet_term_ctx (AD_CTX *ad_ctx);
ST_RET clnp_snet_read_ctx (AD_CTX *ad_ctx, SN_UNITDATA *sn_req);
ST_RET clnp_snet_read_raw_ctx (AD_CTX *ad_ctx,
  ST_UCHAR **pFramePtr,     /* Pointer to pointer to complete frame including MACs*/
  size_t *pFrameLen,        /* Pointer to length of complete frame             */
  struct timeval *pTimestamp);  /* Pointer to timestamp                        */
ST_RET clnp_snet_write_raw_ctx (AD_CTX *ad_ctx,
  ST_UCHAR *framePtr,     /* Pointer to complete frame including MACs*/
  size_t frameLen);       /* Length of complete frame             */
ST_RET clnp_snet_set_filter_ctx (AD_CTX *ad_ctx, ST_CHAR *filter);
ST_RET clnp_snet_set_multicast_filter_ctx (AD_CTX *ad_ctx, ST_UCHAR *mac_list, ST_INT num_macs);
ST_RET clnp_snet_set_multicast_only_filter_ctx (AD_CTX *ad_ctx, ST_UCHAR *mac_list, ST_INT num_macs);
ST_RET clnp_snet_rx_all_multicast_start_ctx (AD_CTX *ad_ctx);
ST_RET clnp_snet_rx_all_multicast_or_local_ctx (AD_CTX *ad_ctx);
ST_RET clnp_snet_rx_all_multicast_stop_ctx (AD_CTX *ad_ctx);
ST_RET clnp_snet_rx_multicast_stop_ctx (AD_CTX *ad_ctx);

/*----------------------------------------------------------------------*/
/* Context-based Capture functions                                      */
/*----------------------------------------------------------------------*/
#if defined(PCAP_CAPTURE_FILE)

ST_RET pcapCaptureFileOpenCtx (AD_CTX *ad_ctx, ST_CHAR *filename);
ST_RET pcapCaptureEnableCtx (AD_CTX *ad_ctx, ST_BOOLEAN enable);
ST_RET pcapCaptureFileCloseCtx (AD_CTX *ad_ctx);
ST_RET pcapCaptureEntryCtx (AD_CTX *ad_ctx, struct pcap_pkthdr *header, const u_char *data);
