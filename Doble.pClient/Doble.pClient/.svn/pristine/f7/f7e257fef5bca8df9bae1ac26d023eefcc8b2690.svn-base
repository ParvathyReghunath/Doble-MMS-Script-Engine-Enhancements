...
/* --------  ---  ------   -------------------------------------------  */
/* 09/14/15  JRB  Add mp_fdir_resp_bld to build fdir response.          */
...
FDIR_RESP_INFO *mp_fdir_resp_bld (
  FDIR_REQ_INFO *req_info,
  ST_CHAR *base_path,
  ST_CHAR *fs_name,       /* file spec all concatenated into one string */
  ST_CHAR *ca_name,       /* ca name all concatenated into one string   */
  ST_INT max_pdu_size,    /* max PDU size for this connection           */
  ST_INT16 *errclass,     /* to send in error response if SD_FAILURE returned.*/
  ST_INT16 *errcode);      /* to send in error response if SD_FAILURE returned.*/
