...
/* --------  ---  ------   -------------------------------------------  */
/* 07/23/18  JRB  Add cdc to RUNTIME_TYPE & RUNTIME_BUILD_CTXT.         */
/*                Add EncTrkCtlValType to RUNTIME_CTRL.                 */
/* 05/09/16  JRB  Add TrgOps to RUNTIME_TYPE & RUNTIME_BUILD_CTXT.      */
/* 03/07/14  JRB           Add reserved_2 to RUNTIME_CTRL.              */
/* 06/29/12  JRB           Add ms_local_to_asn1_maxlen.                 */
...
/* Defines for IEC 61850 CDCs. Need to store one of these values in the */
/* "cdc" member of the RUNTIME_TYPE defining each attribute of the CDC. */
/* Currently only the Control CDCs need to be identified.   */
#define CDC_UNKNOWN   0
#define CDC_SPC       1
#define CDC_DPC       2
#define CDC_INC       3
#define CDC_ENC       4
...
struct runtime_type
  ...
  ST_UINT8 TrgOps;      /* TrgOps from SCL for this element             */
  ST_UINT8 cdc;         /* cdc    from SCL for this element             */
...
typedef struct
  ...
  ST_VOID *reserved_2;          /* ptr to any optional info for this type*/
  ST_CHAR *EncTrkCtlValType [MAX_ENC_TRK];  /* type of ctlVal in each EncTrk*/
                        /* array entry is NULL if associated EncTrk NOT present*/
  } RUNTIME_CTRL;
...
typedef struct
  ...
  ST_INT max_count;             /* max count of RUNTIME_TYPE elems      */
                                /* (used only when do_count=SD_TRUE)    */
  ST_UINT8 TrgOps;      /* TrgOps from SCL to copy to each RUNTIME_TYPE */
  ST_UINT8 cdc;         /* cdc    from SCL to copy to each RUNTIME_TYPE */
  } RUNTIME_BUILD_CTXT;
...
ST_UCHAR *ms_local_to_asn1_easy (SD_CONST RUNTIME_TYPE *rt_head, ST_INT rt_num,
                        ...
                        ST_INT asn1BufLen,
                        ST_INT *asn1LenOut);
ST_RET ms_local_to_asn1_maxlen (ASN1_ENC_CTXT *aCtx,
        SD_CONST RUNTIME_TYPE *rt_head,
        ST_INT rt_num,
        ST_CHAR *dptr);
