...
/* --------  ---  ------   -------------------------------------------  */
/* 02/21/17  JRB  Support ST_SEMAPHORE functions for linux too.         */
/*                GS_THREAD_CONTEXT: add stopFlag for most systems.     */
/* 05/31/16  EJV  Rem extra typedef ST_THREAD_PROC from previous change.*/
/* 02/02/16  DSF           Implemented a common thread pattern          */
...
#if defined(linux)
#include <semaphore.h>  /* need POSIX semaphores  */
#endif
...
#define ST_THREAD_ID            unsigned
        /* NOTE: Thread Identification returned from _beginthreadex is 'unsigned' type          */
        /*       Thread Identification returned from the GetCurrentThreadId is 'DWORD' type     */
...
typedef ST_BOOLEAN (*ST_THREAD_PROC) (struct gs_thread_context *pContext);
...
typedef struct gs_thread_context
  {
  ST_THREAD_PROC      threadProc;

  ST_THREAD_HANDLE    threadHandle;
  ST_THREAD_ID        threadId;

  ST_EVENT_SEM        startEvent;
  ST_EVENT_SEM        stopEvent;  /* for _WIN32 */
  ST_INT              stopFlag;   /* for most systems */

  ST_CHAR             threadName[MAX_THREAD_NAME + 1];
  ST_VOID             *pUserInfo;

  ST_INT              threadState;
  } GS_THREAD_CONTEXT;

GS_THREAD_CONTEXT *gs_thread_allocate_context ();
ST_VOID gs_thread_release_context (GS_THREAD_CONTEXT *pThreadContext);
ST_RET gs_thread_start (GS_THREAD_CONTEXT *pThreadContext);
ST_RET gs_thread_stop (GS_THREAD_CONTEXT *pThreadContext, ST_LONG timeout);
