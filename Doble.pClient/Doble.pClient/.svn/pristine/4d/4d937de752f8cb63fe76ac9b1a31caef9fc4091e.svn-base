...
/* --------  ---  ------   -------------------------------------------  */
/* 03/24/15  JRB  If SNAP_LITE_THR, add "MMS Lite" in SISCO_REGISTRY_*. */
/* 05/19/14  EJV  Added PTR_U64_UNION (to avoid casting warnings).      */
/* 03/01/12  EJV           _Win32: changed to avoid Klocwork warnings:  */
/*                          ST_INT32  from   signed long to   signed int*/
/*                          ST_UINT32 from unsigned long to unsigned int*/
/* 02/14/12  EJV           Added SISCO_REGISTRY_MMS_EASE_142.           */
/* 04/06/11  EJV           sun: set SD_LITTLE_ENDIAN for Sun x86.       */
/* 03/05/11  EJV           sun,_AIX:corr for 64-bit compilation: changed*/
/*                          ST_INT32  from   signed long to   signed int*/
/*                          ST_UINT32 from unsigned long to unsigned int*/
/* 07/07/10  EJV           Merged SISCO_REGISTRY_SECURITY.              */
/* 08/13/09  EJV     34    Added SISCO_REGISTRY_NETWORK, SISCO_REGISTRY_UIB_ICCP.*/
/* 07/23/09  EJV     33    Added SISCO_REGISTRY_* defs for Vista UAC.   */
/* 06/15/09  EJV     32    linux: corr for 64-bit compilation: changed  */
/*                          ST_INT32  from   signed long to   signed int*/
/*                          ST_UINT32 from unsigned long to unsigned int*/
/* 09/16/08  LWP     31    Ported to QNX Neutrino                       */
...
#else  /* defined(__i386) || defined(__amd64) */
  #define SD_BYTE_ORDER SD_LITTLE_ENDIAN
#endif
...
/************************************************************************/
/************************************************************************/
/* use this union to avoid warnings about casting ST_UINT64 to/from     */
/* (ST_VOID *).                                                         */
typedef union
  {
  ST_VOID   *ptr;
...
/************************************************************************/
/************************************************************************/
/* HKEY_LOCAL_MACHINE registry keys for SISCO's Software installation.  */
/* Note: some keys may also be created under HKEY_CURRENT_USER.         */

#if defined(_WIN32)
  /* registry for SISCO general info                                    */
  #define SISCO_REGISTRY_LICENSE        "SOFTWARE\\SISCO\\License"
  /* registry for SISCO Marben Stack */
  #define SISCO_REGISTRY_OSILL2         "SOFTWARE\\SISCO\\OSILL2\\CurrentVersion"
  /* registry for SISCO SNAP-Lite */
#if defined(SNAP_LITE_THR)
  #define SISCO_REGISTRY_NETWORK        "SOFTWARE\\SISCO\\MMS Lite\\Network\\CurrentVersion"
#else
  #define SISCO_REGISTRY_NETWORK        "SOFTWARE\\SISCO\\Network\\CurrentVersion"
#endif
  /* registry for SISCO products */
  #define SISCO_REGISTRY_MMS_EASE_142   "SOFTWARE\\SISCO\\MMS-EASE-142\\CurrentVersion"
  #define SISCO_REGISTRY_MMS_EASE       "SOFTWARE\\SISCO\\MMS-EASE\\CurrentVersion"
  #define SISCO_REGISTRY_AXS4_MMS       "SOFTWARE\\SISCO\\AX-S4 MMS\\CurrentVersion"
  #define SISCO_REGISTRY_AXS4_ICCP      "SOFTWARE\\SISCO\\AX-S4 ICCP\\CurrentVersion"
  #define SISCO_REGISTRY_ICCP_TOOLKIT   "SOFTWARE\\SISCO\\ICCP Toolkit\\CurrentVersion"
  #define SISCO_REGISTRY_UIB_ICCP       "SOFTWARE\\SISCO\\UIB4\\CurrentVersion\\UIB Adapter for ICCP"
  /* registry for SISCO security */
#if defined(SNAP_LITE_THR)
  #define SISCO_REGISTRY_SECURITY       "SOFTWARE\\SISCO\\MMS Lite\\Security\\CurrentVersion"
#else
  #define SISCO_REGISTRY_SECURITY       "SOFTWARE\\SISCO\\Security\\CurrentVersion"
#endif
