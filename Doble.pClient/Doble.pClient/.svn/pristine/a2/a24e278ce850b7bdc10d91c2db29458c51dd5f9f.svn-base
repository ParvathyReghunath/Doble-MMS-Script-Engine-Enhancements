...
/* --------  ---  ------   -------------------------------------------  */
/* 05/14/18  EJV  SOCK_INFO: added log_enable for filtered logging.     */
/* 11/10/17  EJV  np_connect_req_ipv6: added ipv6ScopeId.               */
/* 10/03/17  EJV  SOCK_INFO: added useIpv6, ipv6Addr fileds.            */
/* 01/09/17  JRB  TP0_SOCK_CTX: add remListenSockIpv6. Add np_connect_req_ipv6.*/
/* 09/07/12  EJV           AIX: defined AF_LOCAL.                       */
...
#if defined(sun) || defined(__hpux) || defined(_AIX)
#if !defined(AF_LOCAL)
...
typedef struct
  ...
  ST_BOOLEAN useIpv6;
  ST_UCHAR   ipv6Addr [16];          /* IPV6 Addr in network byte order */
...
typedef struct tag_tp0_sock_ctx
  ...
  GEN_SOCK      *remListenSock;         /* listen socket for non-secured*/
                                        /* cons from remotes            */
  GEN_SOCK      *remListenSockIpv6;     /* listen socket for IPv6       */
...
SOCK_INFO *np_connect_req (ST_VOID *user_conn_id, ST_ULONG ipAddr, ST_UINT16 rem_port,
                            S_SEC_ENCRYPT_CTRL *encrypt_ctrl);
/* Connect request function for IPv6.  */
SOCK_INFO *np_connect_req_ipv6 (ST_VOID *user_conn_id, ST_UCHAR *ipv6Addr, ST_UINT ipv6ScopeId,
                                ST_UINT16 rem_port, S_SEC_ENCRYPT_CTRL *encrypt_ctrl);
