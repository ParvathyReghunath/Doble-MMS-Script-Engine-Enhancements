...
/* --------  ---  ------   -------------------------------------------  */
/* 02/22/10  JRB           Add "maxsize" arg on 6 "*ToString" functions.*/
/*                         Del unused CalculateTimeZoneOffset.          */
/*                         Del protos for non-existent functions.       */
/* 08/28/08  EJV     18    Added TSTR_DEF_TIME_FORMAT_Ymd_HMS def.      */
/* 08/05/08  EJV     17    Del duplicate proto, re-arranged protos.     */
...
#define TSTR_DEF_TIME_FORMAT_Ymd_HMS   "%Y-%m-%d %H:%M:%S"   /* for strftime, local time                        */
#define TSTR_DEF_TIME_FORMAT_Ymd_HMSZ  "%Y-%m-%d %H:%M:%SZ"  /* for strftime, if Z appended then GMT time       */
...
ST_RET tstrTimeToString    (time_t t, char *dest, size_t maxsize);
ST_RET tstrTimeToStringGmt (time_t t, char *dest, size_t maxsize);

ST_RET tstrTmToString    (struct tm *t, char *dest, size_t maxsize);

#define BTIME_DEF_TIME_FORMAT "%m-%d-%Y %H:%M:%S"

ST_RET Btime6StringToVals (char *src, ST_INT32 *numDays, ST_INT32 *numMSec);
ST_RET Btime6ValsToString (char *dest, size_t maxsize, ST_INT32 numDays, ST_INT32 numMSec);

ST_RET Btime4StringToVals (char *src, ST_INT32 *numMSec);
ST_RET Btime4ValsToString (char *dest, size_t maxsize, ST_INT32 numMSec);

ST_RET UtcStringToVals (char *src, ST_UINT32 *pSecs, ST_UINT32 *pFraction,
                        ST_UINT32 *pQflags );
ST_RET UtcValsToString (char *dest, size_t maxsize, ST_UINT32 secs, ST_UINT32 fraction,
                        ST_UINT32 qflags);
