...
/* --------  ---  ------   ------------------------------               */
/* 02/08/16  EJV  SSC_SNAP_PARAM: chg cpuAffinity from ST_UINT to ST_UINT64.*/
/* 02/17/15  EJV  SSC_SNAP_PARAM: added connectTimeout.                 */
/* 10/10/14  EJV  Add SSC_VERSION_2, SSC_VERSION_LAST, SSC_VERSION_2_TIME. */
/* 12/06/13  EJV  Added _bConfigSnapLite.                               */
/* 09/07/12  EJV           AIX: chg SISCO_PATH_INSTALL from /usr/lpp/sisco.*/
/* 07/25/12  EJV           Added SISCO_CONF_MOSI_PARM define.           */
/* 07/09/12  EJV           SSC_LEANT_PROFILE: del TcpEnable, Tp4*,      */
/*                           Clnp*, NSAP fields (only TCP supported).   */
...
#define SSC_VERSION_2                   2
#define SSC_VERSION_LAST                SSC_VERSION_2
#define SSC_VERSION_TIME                "2009-01-01"   /* SSC_VERSION_1 */
#define SSC_VERSION_2_TIME              "2014-10-10"   /* SSC_VERSION_2 */
...
#define   SNAPL_CFG_APP_NAME            "SNAP-Lite"
  ...
  /* default install path (fixed) on UNIX, Linux (diff on each system)  */
    ...
    #define SISCO_PATH_INSTALL          "/usr/sisco"
  #endif
  #define SISCO_CONF_MOSI_PARM          "/conf/mosi.parm"
  #define SISCO_PATH_LICENSE            "/license/"
  #define SISCO_PATH_NETWORK            "/network/"
  #define SISCO_PATH_SECURITY           "/security/"
...
typedef struct SSC_SNAP_PARAM_tag
  ...
  ST_UINT        connectTimeout;                        /* in milliseconds      */

  /* SNAP-Lite misc parameters */
  ST_UINT64      cpuAffinity;
  } SSC_SNAP_PARAM;
...
extern ST_BOOLEAN _bConfigSnapLite;     /* to skip reading <AR_Name_List> */
