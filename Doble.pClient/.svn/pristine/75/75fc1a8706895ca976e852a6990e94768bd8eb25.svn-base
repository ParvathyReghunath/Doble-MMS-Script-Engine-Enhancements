...
/* --------  ---  ------   -------------------                          */
/* 08/23/18  EJV  S_SEC_VERSION_STR: changed to V3.8301 various fix/chgs*/
/* 07/10/18  EJV  Added defines: S_AR_AUTH_MODE_PASSWORD,               */
/*                               S_AR_AUTH_MODE_UNAUTHENTICATED.        */
/* 06/01/18  EJV  Added SECLOG_FLOWF, SECLOG_DEBUGF filtering macros.   */
/*                Add SECLOG_CNERR macros. Add log_enable to few funcs. */
/* 01/16/18  EJV  S_SEC_VERSION_STR: changed to V3.8300 for IPV6 chg.   */
/* 08/10/17  EJV  S_SEC_VERSION_STR: changed to V3.8199 for IPV6 chg.   */
/* 07/13/17  EJV  Added SECLOG_CDEBUG2.                                 */
/* 05/09/17  EJV  Add more protos for 61850 implementations in ssecusr.c*/
/* 05/01/17  EJV  Added functions for 61850 implementations in ssecusr.c*/
/* 08/15/16  EJV  S_SEC_VERSION_STR: changed to V3.8100 for             */
/*                  MMS-SECURE-142-764 V11.8100                         */
/* 03/31/16  EJV  Added SHA256 TLS 1.2 ciphers.                         */
/*                S_SEC_VERSION_STR: changed to V3.8003                 */
/* 10/29/15  EJV  S_SEC_VERSION_STR: changed to V3.8002 for             */
/*                AXS4-61850-142-095 V7.0000,MMS-SECURE-142-664 V11.8000*/
/* 08/25/15  EJV  Linux/UNIX: Added secPassGetPasswordForPfxFile.       */
/* 12/06/14  EJV  S_SEC_VERSION_STR: changed to V3.8000 for Windows rel */
/*         MMS-SECURE-142-064 V11.8000,AXS4-ICCP-SECURE-142-095 V3.6000.*/
/*                Add SSL_ENGINE_NAME for /MD, /MTd, /MDd compilitions. */
/* 07/22/14  EJV  S_SEC_VERSION_STR: changed to V3.7000 for Windows rel */
/*                MMS-SECURE-142-064 V11.7000.                          */
/* 11/26/13  EJV  S_SEC_VERSION_STR: changed to V3.6000 for             */
/*                  AXS4-ICCP-SECURE-14x-095 V3.6000 release.           */
/* 11/11/13  EJV  Added sSecAssocIndChkEx proto for !defined(MMS_LITE). */
/* 11/01/13  EJV  Add S_AR_AUTH_HASH_LAST; chg S_AR_AUTH_HASH_SHA2_512  */
/*                from 5 to 4 (consecutive numbers).                    */
/* 10/24/13  EJV  S_SEC_LOC_REM_AR: add authHashDetected, authHashToUse.*/
/* 10/21/13  EJV  Added sSecAssocIndChkEx  proto.                       */
/* 06/12/13  EJV  Mv S_SEC_CERT_STATUS here from various mms?usec.h.    */
/*                secManCfgChange now in ssleexports_l.def,rm  dllimport*/
/* 05/21/13  EJV  S_AR_AUTH_MAX_CERTS: chg from 16 to 24 (triple redund)*/
/* 05/16/13  EJV  Added S_SEC_XML_STACK_CFG_STR_* defines.              */
/* 03/25/13  EJV  S_SIGNED_TIME: added Hash field to pass to SSL Engine.*/
/*                Added sSecMaceAuthInfoCreateEx proto.                 */
/*                S_SEC_SPARAMS: added authHash.                        */
/*                sSecAuthChk: added remArSec parameter.                */
/* 02/14/13  EJV  S_SEC_LOC_REM_AR: added authSignatureMethod and       */
/*                authHash for MACE time signing.                       */
/*                Added S_AR_AUTH_SIGNATURE_*,S_AR_AUTH_HASH_* defines. */
/* 01/02/13  EJV  S_SEC_VERSION_STR: changed to V3.3100 for Windows rel */
/*                MMS-SECURE-142-064 V11.3100.                          */
/*                S_CERT_CTRL: added certFound field.                   */
/* 10/18/12  EJV           S_AR_AUTH_MAX_CERTS: chg from 8 to 16.       */
/* 09/07/12  EJV           S_SEC_VERSION_STR: changed to V3.3000 for    */
/*                           MMS-SECURE-142-664 V11.3000.               */
/* 06/20/12  EJV           S_SEC_VERSION_STR: changed to V3.2000 for    */
/*                           MMS-SECURE-142-764 V11.2000.               */
/* 06/05/12  EJV           S_CERT_ID: added comment for certType.       */
/* 02/22/12  EJV           Chg comment: max key size from 1024 to 2048. */
/*                         Added S_CERT_MAX_KEY_SIZE define set to 2048.*/
/*                         Chg S_SEC_MAX_SIGNED_TIME_LEN from 128 to 256*/
...
#define S_SEC_VERSION_MINOR             8301      /* number for easy ver cmp    */
#define S_SEC_VERSION_STR               "3.8301"  /* Security Product version   */

#if defined(_WIN32)
  /* this is new SSL Engine DLL name with SISCO prefix */
  /* This DLL name is used to set source of security event logs "EventMessageFile". */
  #if _DEBUG
    #if defined(_MT) && !defined(_DLL)    /* compiled with /MTd   */
      #define SSL_ENGINE_NAME             "SISCO.Security.SslEngine_ld.dll"
    #endif
    #if defined(_MT) && defined(_DLL)    /* compiled with /MDd   */
      #define SSL_ENGINE_NAME             "SISCO.Security.SslEngine_l_DLLd.dll"
    #endif
  #else
    #if defined(_MT) && !defined(_DLL)    /* compiled with /MT   */
      #define SSL_ENGINE_NAME             "SISCO.Security.SslEngine_l.dll"
    #endif
    #if defined(_MT) && defined(_DLL)    /* compiled with /MD   */
      #define SSL_ENGINE_NAME             "SISCO.Security.SslEngine_l_DLL.dll"
    #endif
  #endif
  #if !defined(SSL_ENGINE_NAME)
    #define SSL_ENGINE_NAME               "SISCO.Security.SslEngine_UNKNOWN.dll"
  #endif
...
/* Maximum signed time data size (for 2048-bit RSA key) */
#define S_SEC_MAX_SIGNED_TIME_LEN 256
...
#define S_HASH_SHA            0x000003  /* SHA same as SHA1 */
#define S_HASH_SHA256         0x000004  /* one of the  SHA2 hashes */
...
/* SISCO disabled weak ciphers in 2003: DH & FORTEZA   */
...
/* Cipher Suites are defined by TLS 1.2 */
#define S_CIPHER_020104  (S_KEYX_RSA          | S_CRYPT_NULL          | S_HASH_SHA256)
#define S_CIPHER_020B04  (S_KEYX_RSA          | S_CRYPT_AES_128_CBC   | S_HASH_SHA256)
#define S_CIPHER_020C04  (S_KEYX_RSA          | S_CRYPT_AES_256_CBC   | S_HASH_SHA256)
...
#define S_CERT_MAX_KEY_SIZE                     2048
...
typedef struct  S_CERT_ID_tag
  ...
  ST_UINT      certType;        /* one of the defined S_CERT_TYPE_*     */
                                /* Set only by seccfg.c V1.3000 and below*/
...
typedef struct  S_SIGNED_TIME_tag
  ...
  ST_INT     hash;                  /* one of the S_AR_AUTH_HASH_* defs */
  } S_SIGNED_TIME;
...
/* Internal use */
  ST_UINT32  cfgId;             /* computed 32-bit CRC on certId        */
                                /* enabled fields                       */
  ST_BOOLEAN certFound;         /* in V11.3100, set  to 1 if config CA found in Cert Store      */
  } S_CERT_CTRL;
...
#define S_AR_AUTH_MODE_PASSWORD         3       /* ACSE password in non-secure mode */
#define S_AR_AUTH_MODE_UNAUTHENTICATED  4       /* accepted w/o any authentication  */
/* For backward compatibility only */
#define S_AR_AUTH_MODE_NONE             S_AR_AUTH_MODE_AETITLE

/* authentication signature methods for MACE certificate signed time.                     */
#define S_AR_AUTH_SIGNATURE_NO_HASH   0    /* don't use hashing before signing MACE time, */
                                           /* (this was the way until V11.3100)           */
#define S_AR_AUTH_SIGNATURE_USE_HASH  1    /* use specified hash when signing MACE time   */
#define S_AR_AUTH_SIGNATURE_AUTO      2    /* default try NO_HASH first, then USE_HASH    */
                                           /* (application implemented option)            */

/* authentication Hash Algorithm for MACE certificate signed time.            */
#define S_AR_AUTH_HASH_NONE      0
#define S_AR_AUTH_HASH_SHA1      1    /* digest size 160-bits                 */
#define S_AR_AUTH_HASH_SHA2_256  2    /* digest size 256-bits (SHA-2 series)  */
#define S_AR_AUTH_HASH_SHA2_384  3    /* digest size 384-bits (SHA-2 series)  */
#define S_AR_AUTH_HASH_SHA2_512  4    /* digest size 512-bits (SHA-2 series)  */
#define S_AR_AUTH_HASH_LAST      4    /* last, highest implememted            */
/* NOTE: the AUTO detection algorithm of hash used for MACE time signature    */
/*       assumes that above hashes are defined in such order that higher      */
/*       number specifies better (more advanced) hash that is acceptable on   */
/*       called side.                                                         */

#define S_MAX_LOCAL_SSL_CERTS           2       /* max local  SSL  certificates */
/* #define S_MAX_LOCAL_MACE_CERTS       S_AR_AUTH_MAX_CERTS     in 142xxx       */
#define S_AR_AUTH_MAX_CERTS             24      /* max remote authentication certs (MACE or SSL) */
...
typedef struct  S_SEC_LOC_REM_AR_tag
  ...
  ST_INT     arAuthMode;
  ST_INT     authSignatureMethod;       /* config MACE signature method */
  ST_INT     authHash;                  /* config MACE signature hash   */
  /* when  authSignatureMethod=S_AR_AUTH_SIGNATURE_AUTO                 */
  ST_INT     authHashDetected;          /* remote hash detected         */
  ST_INT     authHashToUse;             /* saved hash from good connect */
...
#define S_SEC_CHECK_ERR_CERT_KEY_SIZE                   25      /* Certificate Key size is > S_CERT_MAX_KEY_SIZE        */
...
typedef struct  S_SEC_CONFIG_tag
  ...
  ST_INT cpuAffinity;                   /* set SslEngine DLL CPU Affinity - not used */
...
ST_RET sSecAssocIndChkEx (struct mvl_net_info *cc, DIB_MATCH_CTRL *matchCtrl,
                        S_SEC_LOC_REM_AR **arSecOut, S_SEC_AUTHCHK_RSLT *secChkDest, ST_BOOLEAN doArMatching);
...
ST_RET sSecAuthChk (struct mvl_net_info *cc, S_SEC_AUTHCHK_RSLT *srcChkDest, S_SEC_LOC_REM_AR *remArSec);
...
/* Functions for 61850 implementations */
ST_RET sSecAuthChk2 (struct mvl_net_info *cc, S_SEC_AUTHCHK_RSLT *srcChkDest);
ST_RET sSecAssocChk2 (ST_BOOLEAN assocInd, struct mvl_net_info *cc, S_SEC_AUTHCHK_RSLT *secChkDest);
ST_RET sSecAssocConfChk2 (struct mvl_net_info *cc, DIB_MATCH_CTRL *matchCtrl,
                          S_SEC_LOC_REM_AR *remArSec, S_SEC_AUTHCHK_RSLT *secChkDest);                       
ST_RET sSecAssocIndChk2 (struct mvl_net_info *cc, DIB_MATCH_CTRL *matchCtrl,
                         S_SEC_LOC_REM_AR **arSecOut, S_SEC_AUTHCHK_RSLT *secChkDest);
...
ST_RET sSecAssocIndChk (ST_INT chan, DIB_MATCH_CTRL *matchCtrl,
                        S_SEC_LOC_REM_AR **arSecOut, S_SEC_AUTHCHK_RSLT *srcChkDest);
ST_RET sSecAssocIndChkEx (ST_INT chan, DIB_MATCH_CTRL *matchCtrl,
                        S_SEC_LOC_REM_AR **arSecOut, S_SEC_AUTHCHK_RSLT *srcChkDest, ST_BOOLEAN doArMatching);
ST_RET sSecAssocConfChkEx (ST_INT chan, DIB_MATCH_CTRL *matchCtrl,
                           S_SEC_LOC_REM_AR *remArSec, S_SEC_AUTHCHK_RSLT *srcChkDest);
ST_INT sSecAuthChk (ST_INT chan, S_SEC_AUTHCHK_RSLT *srcChkDest, S_SEC_LOC_REM_AR *remArSec);
...
/* Functions for 61850 implementations */
ST_RET sSecAuthChk2 (ST_INT chan, S_SEC_AUTHCHK_RSLT *srcChkDest);
ST_RET sSecAssocChk2 (ST_BOOLEAN assocInd, ST_INT chan, S_SEC_AUTHCHK_RSLT *secChkDest);
ST_RET sSecAssocConfChk2 (ST_INT chan, DIB_MATCH_CTRL *matchCtrl,
                          S_SEC_LOC_REM_AR *remArSec, S_SEC_AUTHCHK_RSLT *secChkDest);
ST_RET sSecAssocIndChk2 (ST_INT chan, DIB_MATCH_CTRL *matchCtrl,
                         S_SEC_LOC_REM_AR **arSecOut, S_SEC_AUTHCHK_RSLT *secChkDest);
...
ST_RET sSecMaceAuthInfoCreateEx (S_SEC_LOC_REM_AR *locArSec, ACSE_AUTH_INFO *authInfo,
                               S_CERT_CTRL **maceCertOut, ST_INT authHash);
/* For backward compatability */
#define sSecMaceAuthInfoCreate(locArSec,authInfo,maceCertOut)  sSecMaceAuthInfoCreateEx(locArSec,authInfo,maceCertOut,S_AR_AUTH_HASH_NONE)
...
ST_RET sSecCertIdToCertCtrl (S_CERT_ID *certId, S_CERT_CTRL **certCtrlOut, ST_BOOLEAN log_enable);
#define  sSecCertIdToRemCertCtrl(certId,certCtrlOut)     sSecCertIdToCertCtrl(certId,certCtrlOut, SD_TRUE)
#define  sSecCertIdToLocCertCtrl(certId,certCtrlOut)     sSecCertIdToCertCtrl(certId,certCtrlOut, SD_TRUE)
...
ST_RET sSecChkIssuer (S_CERT_INFO *certInfo, ST_BOOLEAN log_enable);
...
ST_BOOLEAN sSecCmpIssuer (S_CERT_ID *remCert, S_CERT_ID *trustedCert, ST_BOOLEAN log_enable);
/**** Compare two CertIds ****/
ST_BOOLEAN sSecCmpCertId (S_CERT_ID *certId1, S_CERT_ID *certId2, ST_BOOLEAN log_enable);

/* XML header for temporary file where certificates are writen          */
#define S_SEC_XML_HEADER_STR    "<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n"
#define S_SEC_XML_STACK_CFG_STR_START "<STACK_CFG xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\" xsi:noNamespaceSchemaLocation=\"siscostackcfg.xsd\">\n"
#define S_SEC_XML_STACK_CFG_STR_END   "</STACK_CFG>\n"
...
ST_RET secPassGetPasswordForPfxFile (ST_CHAR *pfxFile, ST_CHAR *certPass, ST_INT certPassLen);
#endif /* !defined(_WIN32) */
...
#define SECLOG_CNERR0(a) \
        SLOGC_0 (sec_debug_sel & SEC_LOG_NERR,a)
#define SECLOG_CNERR1(a,b) \
        SLOGC_1 (sec_debug_sel & SEC_LOG_NERR,a,b)
#define SECLOG_CNERR2(a,b,c) \
        SLOGC_2 (sec_debug_sel & SEC_LOG_NERR,a,b,c)
...
#define SECLOG_FLOWF0(log_enable,a) \
        SLOG_0 ((!sLogCtrl->filtered_logging_enable || log_enable) && (sec_debug_sel & SEC_LOG_FLOW), _sec_flow_logstr, a)
#define SECLOG_FLOWF1(log_enable,a,b) \
        SLOG_1 ((!sLogCtrl->filtered_logging_enable || log_enable) && (sec_debug_sel & SEC_LOG_FLOW), _sec_flow_logstr, a,b)
#define SECLOG_FLOWF2(log_enable,a,b,c) \
        SLOG_2 ((!sLogCtrl->filtered_logging_enable || log_enable) && (sec_debug_sel & SEC_LOG_FLOW), _sec_flow_logstr, a,b,c)
#define SECLOG_FLOWF3(log_enable,a,b,c,d) \
        SLOG_3 ((!sLogCtrl->filtered_logging_enable || log_enable) && (sec_debug_sel & SEC_LOG_FLOW), _sec_flow_logstr, a,b,c,d)

#define SECLOG_CFLOWF0(log_enable,a) \
        SLOGC_0 ((!sLogCtrl->filtered_logging_enable || log_enable) && (sec_debug_sel & SEC_LOG_FLOW),a)
#define SECLOG_CFLOWF1(log_enable,a,b) \
        SLOGC_1 ((!sLogCtrl->filtered_logging_enable || log_enable) && (sec_debug_sel & SEC_LOG_FLOW),a,b)
#define SECLOG_CFLOWF2(log_enable,a,b,c) \
        SLOGC_2 ((!sLogCtrl->filtered_logging_enable || log_enable) && (sec_debug_sel & SEC_LOG_FLOW),a,b,c)
...
#define SECLOG_CDEBUG2(a,b,c) \
        SLOGC_2 (sec_debug_sel & SEC_LOG_DEBUG,a,b,c)

/* filtering macros */

#define SECLOG_DEBUGF0(log_enable,a) \
        SLOG_0 ((!sLogCtrl->filtered_logging_enable || log_enable) && (sec_debug_sel & SEC_LOG_DEBUG), _sec_debug_logstr, a)
#define SECLOG_DEBUGF1(log_enable,a,b) \
        SLOG_1 ((!sLogCtrl->filtered_logging_enable || log_enable) && (sec_debug_sel & SEC_LOG_DEBUG), _sec_debug_logstr, a,b)
#define SECLOG_DEBUGF2(log_enable,a,b,c) \
        SLOG_2 ((!sLogCtrl->filtered_logging_enable || log_enable) && (sec_debug_sel & SEC_LOG_DEBUG), _sec_debug_logstr, a,b,c)
#define SECLOG_DEBUGF3(log_enable,a,b,c,d) \
        SLOG_3 ((!sLogCtrl->filtered_logging_enable || log_enable) && (sec_debug_sel & SEC_LOG_DEBUG), _sec_debug_logstr, a,b,c,d)

#define SECLOG_CDEBUGF0(log_enable,a) \
        SLOGC_0 ((!sLogCtrl->filtered_logging_enable || log_enable) && (sec_debug_sel & SEC_LOG_DEBUG),a)
#define SECLOG_CDEBUGF1(log_enable,a,b) \
        SLOGC_1 ((!sLogCtrl->filtered_logging_enable || log_enable) && (sec_debug_sel & SEC_LOG_DEBUG),a,b)
#define SECLOG_CDEBUGF2(log_enable,a,b,c) \
        SLOGC_2 ((!sLogCtrl->filtered_logging_enable || log_enable) && (sec_debug_sel & SEC_LOG_DEBUG),a,b,c)
