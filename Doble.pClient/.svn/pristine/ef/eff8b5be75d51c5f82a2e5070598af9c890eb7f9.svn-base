...
/* --------  ---  ------   -------------------------------------------  */
/* 07/23/18  JRB  GSE_IEC_CTRL: add objRefAcsi for LTRK.                */
/* 02/21/17  JRB  GSE_IEC_CTRL: add isRgoose flag for Routable GOOSE.   */
/* 05/11/16  DSF           Added gse_iec_enc_part2_nohdr()              */
/* 05/21/14  JRB           GSE_IEC_CTRL: add fixedOffs flag.            */
/* 01/15/14  JRB           Increase appID (GoID) size for Edition 2.    */
/* 06/29/12  JRB           Add gse_iec_encode_maxlen.                   */
...
typedef struct
  ...
  ST_BOOLEAN fixedOffs;           /* if true, encode fixed offset GOOSE */
  ST_BOOLEAN isRgoose;            /* if true, encode R-GOOSE            */
  ST_INT32 numDataEntries;
  GSE_IEC_DATA_ENTRY *dataEntries;      /* array of data entry structs  */
  ST_CHAR objRefAcsi [MVL61850_MAX_OBJREF_LEN + 1]; /* for LTRK */
  }GSE_IEC_CTRL;
...
ST_UCHAR *gse_iec_enc_part2 (GSE_IEC_CTRL *ctrl,
        ...
        ST_INT *asn1_len_out);          /* len of ASN.1 encoded data    */
ST_UCHAR *gse_iec_enc_part2_nohdr (GSE_IEC_CTRL *ctrl,
        ETYPE_INFO *etype_info,         /* ptr to Ethertype info        */
        ST_UCHAR *buf_ptr,              /* buffer in which to encode    */
        ST_INT buf_len,                 /* buffer len                   */
        ST_INT enc_part1_len,           /* len of data encoded by       */
...
/* Similar to gse_iec_encode but ONLY computes maximum encoded length.  */
ST_RET gse_iec_encode_maxlen (GSE_IEC_CTRL *ctrl,
