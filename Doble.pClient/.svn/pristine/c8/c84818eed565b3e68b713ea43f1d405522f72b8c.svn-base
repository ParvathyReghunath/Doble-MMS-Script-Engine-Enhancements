...
/* --------  ---  ------   -------------------------------------------  */
/* 03/27/14  JRB  Chg numBuf, etc. to ST_UINT to allow bigger pools.    */
...
typedef struct _smem_pool_ctrl
  ...
  size_t bufSize;
  ST_UINT numBuf;
  ST_UINT nextAvailBuf;
  ST_UINT *availIndexStack;    /* Index table, size [numBuf] */
  ST_CHAR *firstBuf;
  ST_CHAR *lastBuf;
...
ST_VOID  u_smem_get_pool_params ( SMEM_CONTEXT *contextName,
                                  ST_UINT *numBuf, ST_UINT *bufSize,
                                  ST_BOOLEAN *autoClone,
                                  ST_INT *maxClones,
                                  ST_CHAR **poolName);

SMEM_POOL_CTRL *u_smem_need_buffers (SMEM_CONTEXT *smemContext,
                                     ST_UINT numBuf, size_t bufSize);
