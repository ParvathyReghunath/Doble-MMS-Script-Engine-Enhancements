...
/* --------  ---  ------   -------------------------------------------  */
/* 03/05/14  EJV   Implemented listen off/on operations:                */
/*                   Added SNAPL_IPC_OP_LISTEN_* opcodes.               */
/*                   Added SNAPL_LISTEN_* structs.                      */
...
/* added new opcodes */
#define SNAPL_IPC_OP_LISTEN_OFF         24      /* SNAP-Lite stop  listening on port 102, 3782 */
#define SNAPL_IPC_OP_LISTEN_OFF_ACK     25      /* SNAP-Lite stop  listening ACK               */
#define SNAPL_IPC_OP_LISTEN_ON          26      /* SNAP-Lite start listening on port 102, 3782 */
#define SNAPL_IPC_OP_LISTEN_ON_ACK      27      /* SNAP-Lite start listening ACK               */
...
typedef struct SNAPL_IPC_MSG_tag
  ...
  ST_UINT16 version;                    /* always = SNAPL_IPC_VERMmm            */
...
typedef struct SNAPL_LISTEN_OFF_PARAM_tag
  {
  ST_UINT64     listenOffCbFun;
  } SNAPL_LISTEN_OFF_PARAM;

typedef struct SNAPL_LISTEN_OFF_ACK_PARAM_tag
  {
  ST_UINT64     listenOffCbFun;
  } SNAPL_LISTEN_OFF_ACK_PARAM;

typedef struct SNAPL_LISTEN_ON_PARAM_tag
  {
  ST_UINT64     listenOnCbFun;
  } SNAPL_LISTEN_ON_PARAM;

typedef struct SNAPL_LISTEN_ON_ACK_PARAM_tag
  {
  ST_UINT64     listenOnCbFun;
  } SNAPL_LISTEN_ON_ACK_PARAM;
