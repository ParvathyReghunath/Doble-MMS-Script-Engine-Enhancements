...
/* --------  ---  ------   ------------------------------------------   */
/* 03/02/16  JRB  mvl_ind_rcvd: send error response if maxpend_ind exceeded.*/
...
ST_VOID mvl_ind_rcvd (MVL_COMM_EVENT *event)
  ...
  /* If this would exceed maxpend_ind, send MMS error response.*/
  if (event->net_info->numpend_ind >= event->net_info->maxpend_ind)
    {
    _mvl_send_err_f (event, MMS_ERRCLASS_RESOURCE, MMS_ERRCODE_MEM_UNAVAILABLE);
    return;
    }
