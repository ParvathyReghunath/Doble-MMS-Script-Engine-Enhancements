...
/* --------  ---  ------   -------------------------------------------  */
/* 07/16/18  EJV  Del convertIpAddr,convertIpv6Addr (use lookupIpAddr). */ 
/* 06/14/18  EJV  GEN_SOCK_CONFIG: add log_enable for filtered logging. */
/* 06/15/18  EJV  Added GS_MAX IP_STR_LEN for IPV4 / IPV6 address string*/
/* 05/31/18  EJV  Added lookupIpAddr proto to ret IPv4 or IPv6 address. */
/* 03/21/18  EJV  convertIpv6Addr: chg ipv6ScopeId to ST_UINT like defin*/
/* 11/10/17  EJV  Added sockInitCallingEx.                              */
/* 10/03/17  EJV  More changes for IPv6: added sockInitListenExx,       */
/*                sockStartListenAddr, sockGetRemIpv6AddrInfo,          */
/*                convertIpv6Addr. Added SOCKADDR_IN6 define.           */
/*                GEN_SOCK: rpl 'struct sockaddr_in'  with SOCKADDR_IN. */
/*                GEN_SOCK: added ipv6Addr.                             */
/* 01/09/17  JRB  GEN_SOCK: add useIpv6 flag.                           */
/*                Add sockInitListenAddr function.                      */
/* 02/25/16  JRB  GEN_SOCK: add waitingForTp0Conn flag.                 */
/* 03/13/14  EJV  Added sockStopListen, sockStartListen.                */
/*                Added GS_STATE_STOPPED_LISTEN state define.           */
/* 12/09/13  EJV  Added sockInitListenEx with new param bLoopbackAddr.  */
/* 04/13/13  MDE           Changed GEN_SOCK_WAKEUP_PORT_BASE to 20300   */
...
#define GEN_SOCK_MAX_IP_STR_LEN  45+1   /* longest dotted-decimal IPV4 address or IPV6 address converted to text  */
...
/* Set to enable write select */
  ...
  ST_BOOLEAN    log_enable;      /* enable filtered logging on this con */
  } GEN_SOCK_CONFIG;
...
#define GS_STATE_STOPPED_LISTEN         7
...
/* Internal */
  ...
  ST_BOOLEAN           waitingForTp0Conn;    /* SD_TRUE when waiting    */

  ST_BOOLEAN           useIpv6;  /* SD_TRUE if this socket uses IPv6    */
...
#define GEN_SOCK_WAKEUP_PORT_BASE       20300
#define GEN_SOCK_WAKEUP_PORT_RANGE      100
...
ST_RET sockInitCallingEx (GEN_SOCK_CTXT *sockCtx, GEN_SOCK_CONFIG *sockCfg,
                        ST_UINT16 portNo, ST_CHAR *ipAddr, ST_UINT ipv6ScopeId, GEN_SOCK **pSockOut);
...
ST_RET sockInitListenEx (GEN_SOCK_CTXT *sockCtx, GEN_SOCK_CONFIG *sockCfg,
                       ST_BOOLEAN bLoopbackAddr, ST_UINT16 portNo, ST_INT maxActive,
                       GEN_SOCK **pListenSockOut);
ST_RET sockInitListenExx (GEN_SOCK_CTXT *sockCtx, GEN_SOCK_CONFIG *sockCfg,
                          ST_BOOLEAN bLoopbackAddr, ST_UINT16 portNo, ST_INT maxActive,
                          GEN_SOCK **pListenSockOut, ST_VOID *ipAddr, ST_BOOLEAN useIpv6, ST_UINT ipv6ScopeId);
ST_RET sockInitListenAddr (GEN_SOCK_CTXT *sockCtx, GEN_SOCK_CONFIG *sockCfg,
                           ST_CHAR *ipAddr,     /* IPv4 or IPv6 address string */
		           ST_UINT ipv6ScopeId,
                           ST_UINT16 portNo,
                           ST_INT maxActive,
                           GEN_SOCK **pListenSockOut);
ST_RET sockStopListen (GEN_SOCK_CTXT *sockCtx, GEN_SOCK *pListenSock);
ST_RET sockStartListen (GEN_SOCK_CTXT *sockCtx, GEN_SOCK *pListenSock,
                        ST_BOOLEAN bLoopbackAddr, ST_UINT16 portNo);
ST_RET sockStartListenAddr (GEN_SOCK_CTXT *sockCtx, GEN_SOCK *pListenSock,
                            ST_UINT16 portNo, ST_VOID *ipAddr, ST_BOOLEAN useIpv6, ST_UINT ipv6ScopeId);
...
ST_RET sockGetRemIpv6AddrInfo (GEN_SOCK *pSock, SOCKADDR_IN6 *remSockAddrDest,
                               ST_CHAR *remAddrTxtOut, ST_INT remAddrTxtLen,
                               ST_INT *portOut);
...
ST_RET lookupIpAddr (ST_CHAR  *ipAddrStr, ST_BOOLEAN *isIpv6AddrOut, 
                     ST_VOID *ipAddrOut, ST_UINT *ipv6ScopeIdOut);
