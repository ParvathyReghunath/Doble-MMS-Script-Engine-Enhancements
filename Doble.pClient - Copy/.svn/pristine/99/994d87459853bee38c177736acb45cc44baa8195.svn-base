...
/* --------  ---  ------   -------------------------------------------  */
/* 02/28/18  JRB  Add WARNING comment about changing TP0_CFG.           */
/* 11/10/17  EJV  TP4_ADDR, TP0_CFG: add ipv6ScopeId.                   */
/* 01/09/17  JRB  TP4_ADDR: add useIpv6, ipv6Addr.                      */
/*                Add tp4_connect_ind_ipv6 funct.                       */
/*                TP0_CFG: add listenIpv4AddrText, listenIpv6AddrText.  */
...
#define MAX_IPV4_ADDR_LEN 15    /* longest dotted-decimal address       */
#define MAX_IPV6_ADDR_LEN 45
...
typedef struct tagTP4_ADDR
  ...
  ST_UINT   ipv6ScopeId;    /* Linux: needed if ipv6Addr is link-local  */
...
ST_VOID tp4_connect_ind_ipv6 (ST_VOID *user_bind_id, ST_LONG tp4_conn_id,
                      ST_UCHAR *loc_tsel, ST_UCHAR *rem_tsel,
                      ST_UCHAR *remIpv6Addr, /* 16 byte binary addr*/
                      ST_INT conndata_len, char *conndata);
...
typedef struct
  {
  /* WARNING: CHANGING "TP0_CFG" MAY BREAK initialization of "tp0_cfg" in "tp0main.c".*/
  ST_UINT16 max_tpdu_len;       /* max len of TPDU.  Base on SNPDU size.*/
                        /* Use to allocate TPDU buffers.        */
  ST_UCHAR max_tpdu_len_enc;    /* Binary encoded MAX TPDU len. Computed*/
  ...
  /* These must be set before "copp_initialize".  */
  ST_CHAR listenIpv4AddrText [MAX_IPV4_ADDR_LEN+1];
  ST_CHAR listenIpv6AddrText [MAX_IPV6_ADDR_LEN+1];
  ST_UINT ipv6ScopeId;	        /* Linux: network interface idx         */
