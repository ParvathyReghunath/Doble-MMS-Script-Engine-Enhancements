...
/* --------  ---  ------   -------------------------------------------  */
/* 07/23/18  JRB  Add structures and parameters for LTRK support.       */
/* 07/10/18  EJV  MVL_NET_INFO: added encryptionUsed, authTypeUsed.     */
/* 01/16/18  JRB  MVL_VAR_ASSOC: add extrefArr & extrefCount.           */
/* 01/12/18  JRB  MVL_VAR_ASSOC: add "va_scope".                        */
/* 04/13/17  JRB  MVL_NET_INFO: add file_array for 64-bit systems.      */
/* 03/21/17  JRB  MVL_VAR_ASSOC: add beh_stval_ptr, beh_stval_type.     */
/*                Add mvl61850_get_beh_stval function.                  */
/* 07/27/16  JRB  MVLAS_READ_CTRL: add numVarDone, readLoopDone.        */
/*           JRB  MVLAS_WRITE_CTRL: add numVarDone.                     */
/* 07/15/16  JRB  Add USE_TYPE_PTR option.                              */
/* 05/19/16  JRB  Add function mvl_vmd_nvl_add_aa_2.                    */
/* 07/17/15  JRB  Add mvl_fdir_resp_base_path.                          */
/* 05/11/15  JRB  MVL_DOM_CTRL: add mod_var_count, mod_var_array, etc.  */
/* 11/25/14  JRB  Chg DynDataSetCount to ST_INT.                        */
/* 11/05/14  JRB  Add DynDataSetCount, mvl_vmd_nvl_add_2.               */
/* 06/27/14  JRB  Add RBAC definitions and global variables.            */
/*                Chg loc_ar_sec, rem_ar_sec to (S_SEC_LOC_REM_AR *).   */
/* 03/07/14  JRB  Add ENUM_INFO struct.                                 */
/*                MVL_VMD_CTRL: add enum_info_arr, "DynDataSetMax*".    */
/*                MVLAS_READ_CTRL: add detect_changes.                  */
/*                MVL_JOURNAL_CTRL: add EntryID.                        */
/*                MVL_DOM_CTRL: add lphd_var.                           */
/* 10/10/13  JRB  MVL_VMD_CTRL: added "edition".                        */
/* 04/29/13  EJV  MVL_NET_INFO: added authHash, partAuthHash fields.    */
/* 08/23/12  JRB           Add MVL61850_MAX_OBJREF_LEN_ED1.             */
...
#if 0   /* Enable this define to change all type_id from ST_INT to (MVL_TYPE_CTRL *).*/
#define USE_TYPE_PTR
#endif
...
#define MAX_CNT_SHORT 1000 /*TODO: Arun*/
...
#define MVL_NUM_DYN_DOM_VARS    MAX_CNT /*10*/ /*TODO: Arun*/
...
#define MVL61850_MAX_OBJREF_LEN_ED1     65      /* Tissue 141 not approved for Ed1*/
...
/************************************************************************/
/* RBAC information (from IEC 62351-8).                                 */
/************************************************************************/
#define RBAC_ROLES_MAX  32  /* allows 25 user-defined. Increase if necessary.*/
#define RBAC_RIGHTS_MAX 16  /* allows 5 user-defined. Increase if necessary.*/

/* Predefined Roles defined in IEC 62351-8 (Table 3).  */
#define RBAC_ROLE_VIEWER    0
#define RBAC_ROLE_OPERATOR  1
#define RBAC_ROLE_ENGINEER  2
#define RBAC_ROLE_INSTALLER 3
#define RBAC_ROLE_SECADM    4
#define RBAC_ROLE_SECAUD    5
#define RBAC_ROLE_RBACMNT   6

/* These defines (RBAC_RIGHT_IDX_*) define the index into an array of */
/* booleans to define the rights for each Role.                       */
#define RBAC_RIGHT_IDX_VIEW         0
#define RBAC_RIGHT_IDX_READ         1
#define RBAC_RIGHT_IDX_DATASET      2
#define RBAC_RIGHT_IDX_REPORTING    3
#define RBAC_RIGHT_IDX_FILEREAD     4
#define RBAC_RIGHT_IDX_FILEWRITE    5
#define RBAC_RIGHT_IDX_FILEMNGT     6
#define RBAC_RIGHT_IDX_CONTROL      7
#define RBAC_RIGHT_IDX_CONFIG       8
#define RBAC_RIGHT_IDX_SETTINGGROUP 9
#define RBAC_RIGHT_IDX_SECURITY     10

/* rbacRoleRightMap: two dimensional array.
 * One "row" for each Role. One "column" for each Right.
 * The Role number is used as the index to a "row".
 * Once the Role is selected, that "row" can be copied to the connection
 * structure to indicate all the Rights for that connection.
 * Unused Roles should have all Rights = 0 (i.e. the whole row is 0).
 * Selecting an unused Role will result in NO Rights.
 */
extern ST_BOOLEAN rbacRoleRightMap [RBAC_ROLES_MAX][RBAC_RIGHTS_MAX];
extern ST_CHAR *rbacRoleText [RBAC_ROLES_MAX];

/************************************************************************/
/*                      LTRK_URCB_TRK                                   */
/* Pointers to Tracking data for an IEC 61850 URCB.                     */
/* These point directly to the MMS data accessed by "leaf" functions.   */
/************************************************************************/
typedef struct
  {
  /* CST attributes (common to all tracking objects).  */
  ST_CHAR      *objRef;
  ST_INT8      *serviceType;
  ST_INT8      *errorCode;    /* 61850-7-2 says type is ServiceError  */
  MMS_OVSTRING *originatorID; /* OVstring64 */
  MMS_UTC_TIME *t;            /* time of completion of the service    */

  /* Attributes only in UTS.  */
  ST_CHAR      *rptID;        /* MVL61850_MAX_OBJREF_LEN  */
  ST_BOOLEAN   *rptEna;
  ST_BOOLEAN   *resv;
  ST_CHAR      *datSet;       /* MVL61850_MAX_OBJREF_LEN  */
  ST_UINT32    *confRev;
  MMS_BVSTRING *optFlds;      /* BVstring10 */
  ST_UINT32    *bufTm;
  ST_UINT16    *sqNum;
  MMS_BVSTRING *trgOps;       /* BVstring6  */
  ST_UINT32    *intgPd;
  ST_BOOLEAN   *gi;
  } LTRK_URCB_TRK;
...
/************************************************************************/
/*                      LTRK_BRCB_TRK                                   */
/* Pointers to Tracking data for an IEC 61850 BRCB.                     */
/* These point directly to the MMS data accessed by "leaf" functions.   */
/************************************************************************/
typedef struct
  {
  ...
  ST_UINT32    *intgPd;
  ST_BOOLEAN   *gi;
  /* NOTE: These are in LTRK_BRCB_TRK but not in LTRK_URCB_TRK. */
  ST_BOOLEAN   *purgeBuf;
  ST_UINT8     *entryID;  /* should always be Ostring8. */
...
/************************************************************************/
/*                      LTRK_LOCB_TRK                                   */
/* Pointers to Tracking data for an IEC 61850 LCB.                      */
/* These point directly to the MMS data accessed by "leaf" functions.   */
/************************************************************************/
typedef struct
  {
  ...
  MMS_BTIME6   *oldEntrTm;
  MMS_BTIME6   *newEntrTm;
...
/************************************************************************/
/*                      LTRK_GOCB_TRK                                   */
/* Pointers to Tracking data for an IEC 61850 GCB.                      */
/* These point directly to the MMS data accessed by "leaf" functions.   */
/************************************************************************/
typedef struct
  {
  ...
  /* Attributes only in GTS.  */
  ST_BOOLEAN   *goEna;
  ST_CHAR      *goID;         /* MVL61850_MAX_OBJREF_LEN  */
  ST_CHAR      *datSet;       /* MVL61850_MAX_OBJREF_LEN  */
  ST_UINT32    *confRev;
  ST_BOOLEAN   *ndsCom;
...
/************************************************************************/
/*                      LTRK_MSVCB_TRK                                  */
/* Pointers to Tracking data for an IEC 61850 MSVCB (or USVCB).         */
/* These point directly to the MMS data accessed by "leaf" functions.   */
/************************************************************************/
typedef struct
  {
  ...
  /* Attributes only in MTS.  */
  ST_BOOLEAN   *svEna;
  ST_CHAR      *msvID;        /* MVL61850_MAX_OBJREF_LEN  */
  ST_CHAR      *datSet;       /* MVL61850_MAX_OBJREF_LEN  */
  ST_UINT32    *confRev;
  ST_BOOLEAN   *smpMod;
  ST_UINT16    *smpRate;
  MMS_BVSTRING *optFlds;      /* BVstring5  */
  /* NOTE: these point to data in the dstAddress sub-structure.*/
  ST_UINT8     *dstAddress_Addr;  /* Ostring6 */
  ST_UINT8     *dstAddress_PRIORITY;
  ST_UINT16    *dstAddress_VID;
  ST_UINT16    *dstAddress_APPID;
  ST_UINT16    *noASDU; /* "noASDU" is NOT in 61850-7-2 but mapped in 61850-8-1.*/
                        /* This ptr is set only if "noASDU" is in SCL file.*/
  } LTRK_MSVCB_TRK;

/************************************************************************/
/*                      LTRK_SGCB_TRK                                   */
/* Pointers to Tracking data for an IEC 61850 SGCB.                     */
/* These point directly to the MMS data accessed by "leaf" functions.   */
/************************************************************************/
typedef struct
  {
  ...
  /* Attributes only in STS.  */
  ST_UINT8     *numOfSG;
  ST_UINT8     *actSG;
  ST_UINT8     *editSG;
  ST_BOOLEAN   *cnfEdit;
  MMS_UTC_TIME *lActTim;
  ST_UINT16    *resvTms;
  } LTRK_SGCB_TRK;

/************************************************************************/
/*                      LTRK_GEN_TRK                                    */
/* Pointers to Tracking data for an IEC 61850 GenTrk.                   */
/* These point directly to the MMS data accessed by "leaf" functions.   */
/************************************************************************/
typedef struct
  {
...
/************************************************************************/
/*                      LTRK_CTL_TRK                                    */
/* Pointers to Tracking data for ANY IEC 61850 Control.                 */
/* These point directly to the MMS data accessed by "leaf" functions.   */
/* IMPORTANT: The "ctlVal" attribute is a void pointer. It must be cast */
/*            to the appropriate type for each Tracking object.         */
/************************************************************************/
typedef struct
  {
  /* CST attributes (common to all tracking objects).  */
  ...
  /* Attributes only in CTS.  */
  ST_VOID      *ctlVal; /* different type for each Control. Cast as needed.*/
  MMS_UTC_TIME *operTm;

  /* Need pointer to each attribute of "origin" structure.  */
  ST_INT8      *origin_orCat;
  MMS_OVSTRING *origin_orIdent; /* OVstring64 */

  ST_UINT8     *ctlNum;
  MMS_UTC_TIME *T;
  ST_BOOLEAN   *Test;
  MMS_BVSTRING *Check;          /* BVstring2  */
  ST_UINT8     *respAddCause;
  } LTRK_CTL_TRK;

/************************************************************************/
/* Set one of the boolean flags in this structure when the              */
/* corresponding Tracking object is updated.                            */
/************************************************************************/
typedef struct
  {
  /* In same order as DO list in IEC 61850-7-4  */
  ST_BOOLEAN SpcTrk;
  ST_BOOLEAN DpcTrk;
  ST_BOOLEAN IncTrk;
  ST_BOOLEAN EncTrkArr [MAX_ENC_TRK]; /* array for multiple EncTrk    */
  ST_BOOLEAN ApcFTrk;
  ST_BOOLEAN ApcIntTrk;
  ST_BOOLEAN BscTrk;
  ST_BOOLEAN IscTrk;
  ST_BOOLEAN BacTrk;
  ST_BOOLEAN GenTrk;
...
/************************************************************************/
/*                      LTRK_INFO                                       */
/* This structure allocated in MVL_VAR_ASSOC only if variable is LTRK.  */
/* Contains all important information for tracking.                     */
/************************************************************************/
typedef struct
  {
  LTRK_UPDATED ltrkUpdated; /* flags to indicate which LTRK object was updated*/

  /* Allocate each structure only if it is defined in LTRK. */
  LTRK_CTL_TRK *SpcTrk;
  LTRK_CTL_TRK *DpcTrk;
  LTRK_CTL_TRK *IncTrk;
  LTRK_CTL_TRK *EncTrkArr [MAX_ENC_TRK]; /* array for multiple EncTrk    */
  LTRK_CTL_TRK *ApcFTrk;
  LTRK_CTL_TRK *ApcIntTrk;
  LTRK_CTL_TRK *BscTrk;
  LTRK_CTL_TRK *IscTrk;
  LTRK_CTL_TRK *BacTrk;
  LTRK_GEN_TRK     *GenTrk;
  ...
  LTRK_MSVCB_TRK   *MsvcbTrk;
  LTRK_MSVCB_TRK   *UsvcbTrk;  /* uses same structure as MsvcbTrk */
  LTRK_SGCB_TRK    *SgcbTrk;
  } LTRK_INFO;

/************************************************************************/
/* ENUM_INFO: structure to store EnumType values from SCL file.         */
/************************************************************************/
typedef struct
  {
  ST_UINT val_count;    /* number of Enum values for this EnumType      */
  ST_INT8 *val_arr;     /* array of Enum values for this EnumType       */
  ST_CHAR *id;          /* SCL "id" value for this EnumType             */
  } ENUM_INFO;
...
#define MAX_FILES_OPEN_PER_CONN  10
...
typedef struct mvl_net_info
  ...
  S_SEC_LOC_REM_AR *loc_ar_sec;
  S_SEC_LOC_REM_AR *rem_ar_sec;

  S_CERT_CTRL      *loc_mace;   /* loc MACE cert used in this connection*/
  S_CERT_CTRL      *rem_mace;   /* rem MACE cert used in this connection*/

  ST_INT         authHash;      /* hash used in signed local MACE time     */
  ST_INT         partAuthHash;  /* hash used in signed partner's MACE time */

  /* for fast access */
  ST_BOOLEAN     encryptionUsed; /* 1 if encrypted connection                             */
  ST_INT         authTypeUsed;   /* actual auth that was used in connection (S_AR_AUTH_*) */

  ST_INT16 rbacRole;                      /* RBAC Role for this connection*/
  ST_BOOLEAN rbacRights[RBAC_RIGHTS_MAX]; /* RBAC Rights for this connection*/

  ST_DOUBLE         certExpirationLastCheckTime;

  ST_VOID *user_info;           /* MVL user can use this for 'whatever' */

  /* "file_list" is for BACKWARD COMPATIBLITY ONLY. Will fail on 64-bit */
  /* system when casting (MVL_NET_FILE *) to (ST_INT32) for frsmid.     */
  /* "file_array" should be used instead. */
  MVL_NET_FILE *file_list;      /* linked list of open files on this conn*/
  FILE *file_array [MAX_FILES_OPEN_PER_CONN]; /* open files on this conn*/
  } MVL_NET_INFO;
...
/* MVL_SCOPE                                                            */
/************************************************************************/
struct mvl_dom_ctrl;   /* forward declaration */
...
#if defined(USE_TYPE_PTR)
typedef MVL_TYPE_CTRL *MVL_TYPE_ID;
#define MVL_TYPE_ID_INVALID  NULL
#else
typedef ST_INT MVL_TYPE_ID;
#define MVL_TYPE_ID_INVALID  (-1)
#endif
...
ST_RET mvl_get_runtime (MVL_TYPE_ID type_id,
                       RUNTIME_TYPE **rt_ptr_out,
                       ST_INT *num_rt_out);

/* Same function but only succeeds if type was dynamically created.     */
ST_RET mvl_get_runtime_dyn (MVL_TYPE_ID type_id,
                       RUNTIME_TYPE **rt_ptr_out,
                       ST_INT *num_rt_out);
...
typedef struct mvl_var_assoc
  ...
  MVL_SCOPE va_scope;           /* MMS Object Scope of this variable    */
  ...
  MVL_TYPE_ID type_id;          /* type of variable                     */
  ...
  /* Additional info used ONLY for IEC 61850 variables. */
  ST_VOID *beh_stval_ptr;       /* "Beh.stVal" data ptr                 */
  RUNTIME_TYPE *beh_stval_type; /* "Beh.stVal" type                     */

  ST_VOID **extrefArr;  /* allocated array of pointers (usually only 1) */
                        /* must cast each ptr to (SCL_EXTREF *) to use it */
  ST_UINT extrefCount;  /* number of pointers in the array  */
  } MVL_VAR_ASSOC;
...
typedef struct mvlJournal
  ...
  ST_UCHAR EntryID[8];  /* IEC 61850 EntryID for newest Journal entry.  */
  } MVL_JOURNAL_CTRL;
...
typedef struct mvl_dom_ctrl
  ...
  MVL_VAR_ASSOC *lphd_var;      /* for easy access to LPHD              */
  MVL_VAR_ASSOC *ltrk_var;      /* for easy access to LTRK              */
  LTRK_INFO *ltrk_info;         /* extra info used for LTRK support     */
  ST_VOID *user_info;           /* MVL user can use this for 'whatever' */

  MVL_SCOPE scope;              /* scope of this Domain. Very useful.   */

  /* Variables needed by "mvl61850_update_all_mod_beh".  */
  ST_INT mod_var_count;         /* number of "Mod" variables            */
  MVL_VAR_ASSOC **mod_var_array;/* array of "Mod" variables             */
  MVL_SCOPE **mod_scope_array;  /* array of scope for "Mod" variables.  */
                                /* all entries point to "scope" above.  */
  } MVL_DOM_CTRL;
...
typedef struct mvl_vmd_ctrl
  ...
  ST_INT edition;               /* Edition of standard implemented by this VMD*/

  ST_VOID *user_info;           /* MVL user can use this for 'whatever' */

  /* IEC 61850 parameters.*/
  ENUM_INFO *enum_info_arr;     /* array of structs (one for each EnumType)*/
  ST_INT enum_info_count;       /* num of entries in enum_info_arr      */

  ST_UINT DynDataSetMax;            /* DynDataSet.max from SCL          */
  ST_UINT DynDataSetMaxAttributes;  /* DynDataSet.maxAttributes from SCL*/
  ST_INT  DynDataSetCount;          /* Num of Dynamic data sets in VMD. */
  /* End of IEC 61850 parameters.*/

#if defined(USE_TYPE_PTR)
  MVL_TYPE_CTRL *type_list;     /* linked list of types for this VMD    */
  ST_INT type_count;
  MVL_TYPE_CTRL *named_type_list; /* linked list of "named" types for this VMD*/
  ST_INT named_type_count;
#endif
  } MVL_VMD_CTRL;
...
MVL_VAR_ASSOC *mvl_var_create (OBJECT_NAME *obj, MVL_TYPE_ID type_id,  /* NEW */
...
typedef struct mvlas_read_ctrl
  ...
  ST_BOOLEAN detect_changes;    /* SD_TRUE if leaf functions should check*/
                                /* for changes (dchg, qchg, dupd).      */
  ST_BOOLEAN readLoopDone;      /* looping is done                      */
...
/************************************************************************/
/*                      LTRK_CTL_TRK_VALUES                              */
/* Values saved to update ANY Control object in LTRK.                   */
/************************************************************************/
typedef struct
  {
  MVL_VAR_ASSOC *base_va;
  ST_INT8 errorCode;  /* 61850-7-2: type=ServiceError. 61850-8-1 defines values*/
  ST_INT8 respAddCause;
  ST_INT8 serviceType;
  } LTRK_CTL_TRK_VALUES;
...
typedef struct mvlas_wr_va_ctrl
  ...
  LTRK_CTL_TRK_VALUES CtlTrkValues;  /* to save LTRK attribute values*/
...
ST_RET mvlu_add_rt_type (RUNTIME_TYPE *rt, ST_INT numRt, MVL_TYPE_ID *typeIdOut);
ST_VOID mvlu_free_rt_type (MVL_TYPE_ID typeId);

/* Type to String and visa-versa */
MVL_TYPE_ID mvl_typename_to_typeid (ST_CHAR *type_id_string);
ST_CHAR *mvl_typeid_to_typename (MVL_TYPE_ID type_id);
...
/************************************************************************/
#if defined(USE_TYPE_PTR)
/* These 2 functions replaced with macros (type_id is MVL_TYPE_CTRL *). */
#define mvl_type_ctrl_find(type_id)     type_id
#define mvl_type_ctrl_find_dyn(type_id) type_id
#else
MVL_TYPE_CTRL *mvl_type_ctrl_find (MVL_TYPE_ID TypeId);
MVL_TYPE_CTRL *mvl_type_ctrl_find_dyn (MVL_TYPE_ID TypeId);
#endif

MVL_TYPE_ID mvl_type_id_create (ST_CHAR *type_name, ST_UCHAR *asn1_data, ST_UINT asn1_len);
MVL_TYPE_ID mvl_type_id_create_from_tdl (ST_CHAR *type_name, ST_CHAR *tdl);
ST_VOID mvl_type_id_destroy (MVL_TYPE_ID TypeId);
ST_RET mvl_type_id_destroy_all_dyn (ST_VOID);
ST_INT mvl_type_count (ST_VOID);

/* These take additional "vmd_ctrl" argument.   */
MVL_TYPE_ID mvl_vmd_type_id_create (MVL_VMD_CTRL *vmd_ctrl, ST_CHAR *type_name, RUNTIME_CTRL *rt_ctrl);
ST_RET mvl_vmd_type_id_destroy (MVL_VMD_CTRL *vmd_ctrl, MVL_TYPE_ID TypeId);
ST_RET mvl_vmd_type_id_destroy_all (MVL_VMD_CTRL *vmd_ctrl);
MVL_TYPE_CTRL *mvl_vmd_type_ctrl_find (MVL_VMD_CTRL *vmd_ctrl, MVL_TYPE_ID type_id);
...
ST_INT32 mvl61850_get_beh_stval (MVL_VAR_ASSOC *var);
...
MVL_NVLIST_CTRL *mvl_vmd_nvl_add (MVL_VMD_CTRL *vmd_ctrl,
        ...
        ST_BOOLEAN copy_name);
/* Same as mvl_vmd_nvl_add except includes err_class, err_code. */
MVL_NVLIST_CTRL *mvl_vmd_nvl_add_2 (MVL_VMD_CTRL *vmd_ctrl,
        OBJECT_NAME *nvl_obj,
        MVL_NET_INFO *net_info,
        ST_INT num_var,
        OBJECT_NAME *var_obj,   /* array of var names   */
...
MVL_NVLIST_CTRL *mvl_vmd_nvl_add_aa (MVL_VMD_CTRL *vmd_ctrl,
        ...
        ALT_ACCESS **alt_acc);  /* array of alt acc info for each var*/
                                /* NULL if not present  */
/* Same as mvl_vmd_nvl_add_aa except includes err_class, err_code. */
MVL_NVLIST_CTRL *mvl_vmd_nvl_add_aa_2 (MVL_VMD_CTRL *vmd_ctrl,
        OBJECT_NAME *nvl_obj,
        MVL_NET_INFO *net_info,
        ST_INT num_var,
        OBJECT_NAME *var_obj,   /* array of var names   */
...
/* Base path for FileDirectory response.                                */
/* Must be initialized at startup (default is current directory).       */
extern ST_CHAR mvl_fdir_resp_base_path [MAX_FILE_NAME + 1];
